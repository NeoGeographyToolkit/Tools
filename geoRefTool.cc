...
int main( int argc, char *argv[] )
{
  ...
  general_options.add_options()
    ...
    ("printBounds",  po::bool_switch(&printBounds)->default_value(false),    "Prints the lat/lon bounds")
    ...
    ("outputFormat", po::value<int>(&outputFormat)->default_value(OUTPUT_LAT_LON),
  ...
  if ( !vm.count("inputImage") )
    vw_throw( vw::ArgumentErr() << "Requires <input image> in order to proceed.\n\n"
              << usage << general_options );
  if ( !vm.count("printBounds") && !(vm.count("row") && vm.count("col")) && !(vm.count("lat") && vm.count("lon")) )
    vw_throw( vw::ArgumentErr() << "Requires either printBounds, row/col, or lat/lon in order to proceed.\n\n"
              << usage << general_options );
